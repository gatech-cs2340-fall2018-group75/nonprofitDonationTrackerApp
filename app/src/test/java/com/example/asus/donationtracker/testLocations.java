package com.example.asus.donationtracker;

import com.example.asus.donationtracker.Model.Location;
import com.example.asus.donationtracker.Model.Locations;

import org.junit.Before;
import org.junit.Test;

import java.util.ArrayList;
import java.util.List;

import static com.example.asus.donationtracker.Model.LocationType.DR;
import static org.junit.Assert.*;

public class testLocations {
    public static final int TIMEOUT = 200;
    private static final List<Location> tester = Locations.getInstance();
    List<String> result = new ArrayList<>();

    //tests the "for" branch case where no locations are in the singleton
    @Test(timeout = TIMEOUT)
    public void testNoContents() {
    assertTrue(result.equals(tester.getNames()));
    }

    //tests the "for" branch when items are in the singleton
    @Test(timeout = TIMEOUT)
    public void testWithContents() {
        Location place1 = new Location("DropOff1", DR, 38.8951, 38.8951, "111 temp Way",
                "Fort Myers", "FL", "33993", "(345)135-1468");
        Location RLyeh = new Location("R'lyeh", DR, 47.9, 126.43, "Madness Way",
                "Pacific Ocean","The Beyond","00000", "(366)822-5548");
        tester.
    }
}
